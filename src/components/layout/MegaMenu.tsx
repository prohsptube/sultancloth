        ...
        <div className="flex flex-wrap items-center justify-center">
          {mainNavigation.map((item) => (
            <div key={item.label} className="group relative">
              ...
              <Link
                href={item.href}
                className="flex items-center gap-1.5 whitespace-nowrap px-2 md:px-3 py-2.5 text-[10px] md:text-[11px] font-bold uppercase tracking-[0.15em] text-gray-800 transition hover:text-red-600 relative after:absolute after:bottom-0 after:left-0 after:h-0.5 after:w-0 after:bg-red-600 after:transition-all group-hover:after:w-full"
              >
                {item.label}
                {item.categories && (
              ...
              {/* Mega Menu Dropdown */}
              {item.categories && (
                <div className="absolute left-1/2 -translate-x-1/2 top-full hidden w-screen max-w-4xl bg-white shadow-2xl border-t-4 border-red-600 p-6 group-hover:block">
